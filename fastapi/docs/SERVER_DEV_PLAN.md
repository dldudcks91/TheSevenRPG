...
| `InventoryManager.py` | ✅ 완료 | 장착/해제/조회/판매/인벤확장/장비분해, 코스트 검증, Redis 무효화 (Phase 21) |
| `StageManager.py` | ✅ 완료 | 4웨이브 체크포인트, 귀환, 재접속, 시설해금 (Phase 14+20) |
| `CardManager.py` | ✅ 완료 | 도감 조회 + 카드 인벤토리(조회/분해/레벨업) + 스킬 장착/해제 Cards 기반 (Phase 18.5) |
| `UserInfoManager.py` | ✅ 수정 | 스탯 리셋 API 추가 (Phase 12) (⚠️ idle_farm 참조 코드 제거 필요 — Phase 12.5) |
| `MaterialManager.py` | ✅ 완료 | 재료 조회 + 포션 사용 (Phase 18) |
| `CraftingManager.py` | ✅ 완료 | 매직→크래프트 장비 제작, 낙인+광석+골드 소모 (Phase 19) |
| `ShopManager.py` | ✅ 완료 | 상인 — 골드로 포션/광석 구매 (Phase 20) |
| `QuestManager.py` | ✅ 완료 | 퀘스트 게시판 — 재료 납품→보상 (Phase 20) |
...
| 2007 | CardManager | `get_collection` | 도감 목록 조회 | ✅ 완성 |
| 2008 | CardManager | `equip_skill` | 카드 스킬 슬롯 장착 (Cards 기반) | ✅ 완성 |
| 2009 | CardManager | `unequip_skill` | 카드 스킬 슬롯 해제 (Cards 기반) | ✅ 완성 |
| 2010 | InventoryManager | `disassemble_item` | 장비 분해 → 광석 + 골드 | ✅ 완성 |
| 2011 | MaterialManager | `use_potion` | 포션 사용 (웨이브 클리어 후) | ✅ 완성 |
| 2012 | CardManager | `get_cards` | 카드 인벤토리 조회 | ✅ 완성 |
| 2013 | CardManager | `disassemble_card` | 카드 분해 → 카드 영혼 | ✅ 완성 |
| 2014 | CardManager | `level_up_card` | 카드 레벨업 | ✅ 완성 |
| 2015 | MaterialManager | `get_materials` | 재료 인벤토리 조회 | ✅ 완성 |
...
| 4001 | CraftingManager | `craft_item` | 크래프팅 (매직→크래프트) | ✅ 완성 |
| 4002 | ShopManager | `buy_item` | 상인 구매 | ✅ 완성 |
| 4003 | QuestManager | `submit_quest` | 퀘스트 납품 | ✅ 완성 |
| 4004 | QuestManager | `get_quests` | 퀘스트 목록 조회 | ✅ 완성 |
| 4005 | ShopManager | `get_shop` | 상점 목록 조회 | ✅ 완성 |
...
| ~~2010~~ | ~~InventoryManager~~ | ~~`disassemble_item`~~ | ~~장비 분해~~ | ✅ 21 완료 |
| ~~2011~~ | ~~MaterialManager~~ | ~~`use_potion`~~ | ~~포션 사용~~ | ✅ 18 완료 |
| ~~2012~~ | ~~CardManager~~ | ~~`get_cards`~~ | ~~카드 인벤토리 조회~~ | ✅ 18.5 완료 |
| ~~2013~~ | ~~CardManager~~ | ~~`disassemble_card`~~ | ~~카드 분해 → 카드 영혼~~ | ✅ 18.5 완료 |
| ~~2014~~ | ~~CardManager~~ | ~~`level_up_card`~~ | ~~카드 레벨업 (동일카드N+영혼N)~~ | ✅ 18.5 완료 |
| ~~2015~~ | ~~MaterialManager~~ | ~~`get_materials`~~ | ~~재료 인벤토리 조회~~ | ✅ 18 완료 |
| ~~4001~~ | ~~CraftingManager~~ | ~~`craft_item`~~ | ~~크래프팅~~ | ✅ 19 완료 |
| ~~4002~~ | ~~ShopManager~~ | ~~`buy_item`~~ | ~~상인 구매~~ | ✅ 20 완료 |
| ~~4003~~ | ~~QuestManager~~ | ~~`submit_quest`~~ | ~~퀘스트 재료 납품~~ | ✅ 20 완료 |
...
Phase 17   (드롭 시스템 v2) ✅ 완료 ← ilvl/mlvl/dlvl + 5종 드롭 + 서버사이드 저장
    ↓
Phase 18   (재료 아이템 관리) ✅ 완료 ← get_materials + use_potion + 폴백 상수
    ↓
Phase 18.5 (카드 인벤토리 시스템) ✅ 완료 ← 카드 조회/분해/레벨업 + equip/unequip Cards 전환
    ↓
Phase 19   (크래프팅) ✅ 완료 ← 매직→크래프트, 낙인+광석+골드 소모
    ↓
Phase 20   (NPC 시설) ✅ 완료 ← 상인(구매) + 퀘스트(납품) + 시설해금 로직
    ↓
Phase 21   (장비 분해) ✅ 완료 ← disassemble_item (등급별 광석+골드 회수)
    ↓
...
- `models.py` (수정 — BattleSession.potion_used 추가)
- `services/system/ErrorCode.py` (수정 — E5001~E5003 추가)
- `services/system/APIManager.py` (수정 — API 2011, 2015 등록)

**작업 내용**
- [x] `get_materials` (API 2015): 재료 인벤토리 조회
- [x] `use_potion` (API 2011): 웨이브 클리어 후 HP 회복 (전투 중 불가)
  - 지참 제한: MAX_POTION_PER_RUN = 3 (폴백 상수, 추후 CSV 교체)
  - 포션 3종: 하급(50HP), 중급(150HP), 상급(400HP) — 폴백 상수
- [ ] 광석 등급 계층: 합성 로직 (수치 미확정 → Phase 19)
- [ ] 카드 영혼: 카드 분해 시 획득하는 범용 재료 (→ Phase 18.5)
...
**변경 파일 (5개)**
- `services/rpg/CardManager.py` (전면 재작성 — 도감 + 카드 인벤토리 + 스킬 장착)
- `services/system/GameDataManager.py` (수정 — card_skill.csv 로더 추가)
- `services/system/APIManager.py` (수정 — API 2012, 2013, 2014 등록)
- `services/system/ErrorCode.py` (수정 — E6xxx 카드 에러 코드 추가)
- `services/rpg/StageManager.py` (수정 — 카드 드롭 시 card_stats 랜덤 생성)

**구현 완료 항목**
- [x] `get_cards` (API 2012): 카드 인벤토리 조회 + card_skill 메타데이터 매핑
- [x] `disassemble_card` (API 2013): 카드 분해 → 카드 영혼 (Lv1=1, Lv2=3, Lv3=7)
- [x] `level_up_card` (API 2014): 동일 카드 N장 + 카드 영혼 N개 → 최대 Lv3
- [x] equip_skill/unequip_skill: Collection.skill_slot → Card.is_equipped/skill_slot 전환
- [x] 카드 드롭 시 card_stats 랜덤 생성 (trigger_rate ±10% 변동)
- [x] card_skill.csv → GameDataManager.REQUIRE_CONFIGS['card_skills'] 로드

**미구현 (후속 Phase)**
- 카드 스킬 전투 발동 (BattleManager 연동) — Phase 16 보완
- 도감 그룹 보너스 재설계 — 기획 미확정
...
**구현 완료 항목**
- [x] `craft_item` (API 4001): 매직 장비 + 낙인 + 광석 + 골드 → 크래프트 등급
- [x] 시설 해금 검증 (unlocked_facilities 비트마스크)
- [x] 낙인 소모 (죄종별 접사 롤) + 광석 소모 (item_level×2) + 골드 소모 (item_level×50)

**미구현 (기획 확정 대기)**
- 크래프트 전용 옵션 목록 (현재는 일반 접사풀 사용)
...
- `services/rpg/StageManager.py` (수정 — 시설 해금 로직)

**시설 해금 비트마스크 (StageManager.clear_stage 연동)**
| 해금 조건 | 시설 | 비트 | 상태 |
|----------|------|------|------|
| Ch1 클리어 | 대장간 | 0 | ✅ |
| Ch2 클리어 | 퀘스트 게시판 | 1 | ✅ |
| Ch3 클리어 | 상인 | 2 | ✅ |
| Ch4 클리어 | 흔적 조합소 | 3 | 비트 할당만 (기능 미구현) |

**구현 완료 항목**
- [x] `get_shop` (API 4005): 상점 목록 조회
- [x] `buy_item` (API 4002): 골드로 포션/광석 구매 (5종 상품)
- [x] `get_quests` (API 4004): 퀘스트 목록 + 완료 가능 여부
- [x] `submit_quest` (API 4003): 재료 납품 → 골드/재료 보상 (3종 퀘스트)
- [x] 시설 해금: StageManager.clear_stage에서 챕터 보스 클리어 시 비트 설정

**미구현**
- 흔적 조합소 (Ch4, 상세 기획 미확정)
...
**구현 완료 항목**
- [x] `disassemble_item` (API 2010): 장비 분해 → 광석 + 골드
  - 장착 중 분해 차단
...
| ~~카드 스킬 메타데이터~~ | Phase 18.5 | card_skill.csv 16종 로드, trigger_rate ±10% 랜덤 |
| ~~크래프팅 기본 흐름~~ | Phase 19 | 매직+낙인+광석+골드→크래프트 확정 |
| ~~시설 해금 체계~~ | Phase 20 | Ch1~4 비트마스크 확정, 챕보 클리어 시 자동 해금 |
| ~~장비 분해 체계~~ | Phase 21 | 등급별 광석(magic×1~unique×5)+골드(lv×20) 회수 확정 |

### 미확정 (기획 필요)
| 항목 | 영향 Phase | 비고 |
|------|-----------|------|
| 접사 qlvl 체계 | Phase 17 보완 | mlvl에 따른 접사 수치 스케일링 (현재 min~max 전체) |
| 포션 종류/효과/가격/지참 제한 | Phase 18 | 폴백 상수 구현 (3종, 런당 3회), CSV 교체 대기 |
| 광석 등급 계층/합성 비율 | Phase 19 | 현재 tier 1 통일, 등급 체계 미기획 |
| 카드 레벨업 소모량 (동일카드 N장, 영혼 N개) | Phase 18.5 | 폴백 상수 구현 (Lv1→2: 카드2+영혼3, Lv2→3: 카드4+영혼8) |
| 크래프팅 광석/골드 소모 공식 | Phase 19 | 폴백 상수 구현 (광석=ilvl×2, 골드=ilvl×50) |
| 크래프트 전용 옵션 목록 | Phase 19 | 현재 일반 접사풀 사용, 전용 옵션 미기획 |
| 퀘스트 재료 종류/NPC 보상 | Phase 20 | 폴백 상수 구현 (3종 퀘스트), CSV 교체 대기 |
| 장비 분해 광석 회수량 | Phase 21 | 폴백 상수 구현 (magic×1~unique×5 × ilvl) |
...
| ~~`CollectionManager`~~ | ~~도감 레벨업 전제 설계 + 스킬슬롯~~ | ✅ Phase 18.5 완료 (Cards 기반 전환) |
| `Collections.collection_level` | 도감 레벨업 폐기됨, 컬럼 미사용 | Phase 18.5 (컬럼 존재, 미사용) |
| `Collections.skill_slot` | Cards.skill_slot로 이관 완료, 컬럼 미사용 | Phase 18.5 (컬럼 존재, 미사용) |
